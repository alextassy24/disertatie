    ...
    <AuthenticatedWrapper>
        <div class="mt-28">
            ...
            <section class="container w-full px-10 py-16 mx-auto md:w-3/4 ">
                <div v-if="successMessage" class="success-message animate__animated"
                ...
                <div v-if="wearers.length > 0">
                    <PaginatedTable :headers="headers" :items="wearers" :page="page" :action="toggleModal" :link="link"
                        :item-page="itemPage" />
                    <BaseModal :modalActive="modalActive" @close-modal="toggleModal(null)" @submit-modal="submitRequest">
                        <input type="hidden" v-model="deletedId">
                        <h1 class="text-lg text-black">{{ translatedValues.deleteItem }} {{ deletedId }}?</h1>
                    </BaseModal>
                </div>
                <div v-else>
                    <div
                    ...
                    <div class="flex items-center justify-between w-full gap-3 mx-auto mt-8 md:w-1/2">
                        <span>{{ translatedValues.addWearers }}</span>
                        <router-link to="/register-wearer" class="flex items-center gap-2 btn">
...
const headers = ref(['Nr. Crt.', translatedValues.value.fullName, translatedValues.value.age, ''])
