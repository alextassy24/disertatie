    ...
    <NotAuthenticatedWrapper>
        <section class="container px-10 py-16 mx-auto mt-28">
            ...
            <div
                ...
                <div v-else
					class="w-3/4 p-3 mb-10 font-bold text-center text-green-600 bg-green-200 rounded-lg shadow">{{
						successMessage }}
				</div>
            </div>
        </section>
...
const translatedValues = computed(() => {
	return {
        ...
        successMessage: t("forgot-password.SuccessMessage")
	};
});
const email = ref("");
...
function onSubmit(){
	...
	axios
		.post("http://127.0.0.1:5088/api/account/forgot-password", formData)
        .then((response)=>{
