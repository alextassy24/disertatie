	...
	<div v-if="authStore.isAuthenticated" class="mt-28">
		...
		<section class="container px-10 py-16 mx-auto">
			<div v-if="wearers.length > 0"
				...
				:class="!successMessage ? 'bg-gray-200 rounded-xl shadow-lg' : 'bg-white'">
				...
				<Form v-if="!successMessage" :validation-schema="validationSchema" @submit="onSubmit" class="mb-3">
					<div class="relative flex flex-col mb-5">
						<label class="form-label">{{ translatedValues.deviceSerialNumber }}</label>
						<Field id="deviceSerialNumber" type="text" name="deviceSerialNumber" class="form-field"
					...
					<div class="relative flex flex-col mb-5">
						...
						<Field id="deviceGUID" type="text" name="deviceGUID" class="form-field" v-model="deviceGUID" />
						<ErrorMessage name="deviceGUID" class="error-message" />
					</div>
					<div class="relative flex flex-col mb-5">
						<label class="form-label">{{ translatedValues.wearer }}</label>
						<select id="wearer" name="wearer" class="form-field" v-model="wearer">
							<option v-for="item in wearers" :value="item.id">{{ item.name }}</option>
						</select>
				...
				<div v-else class="p-3 mb-10 font-bold text-center text-green-600 bg-green-200 rounded-lg shadow">
					{{ successMessage }}
				</div>
			</div>
			<div v-else>
				...
				<div
					...
					{{ translatedValues.needWearers }}</div>
			</div>
		</section>
	</div>
...
const validationSchema = toTypedSchema(
	z.object({
		deviceGUID: z
			...
			.refine((value) => /^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/i.test(value), {
...
const getWearers = () => {
	axios
		.get("http://127.0.0.1:5088/api/wearers/", config)
		.then((response) => {
