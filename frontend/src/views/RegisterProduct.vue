	...
	const translatedValues = computed(() => {
		return {
			...
			deviceSerialNumberError: {
				required: t(
					"register-product.DeviceSerialNumberError.Required"
				),
			},
			deviceGUIDError: {
	...
	const validationSchema = toTypedSchema(
		z.object({
			...
			deviceSerialNumber: z
				...
				.min(
					1,
					translatedValues.value.deviceSerialNumberError.required
				),
		})
	);
			...
			.catch((error) => {
				...
				if (error.response.status) {
					switch (error.response.status) {
						case 400:
							errorMessage.value =
								translatedValues.value.badRequest;
							break;
						case 401:
							errorMessage.value =
								translatedValues.value.unauthorized;
							break;
						case 404:
							errorMessage.value =
								translatedValues.value.notFound;
							break;
					}
				}
			});
