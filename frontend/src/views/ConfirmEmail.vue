	...
	<NotAuthenticatedWrapper>
		<div class="mt-28">
			<section class="px-10 py-16">
				...
				<div
					v-else
					class="w-2/4 p-5 mx-auto text-xl font-bold text-center text-red-500 bg-red-200 rounded shadow"
				>
					{{ errorMessage }}
				</div>
			</section>
...
<script setup>
	...
	const confirmEmail = async () => {
		await axios
			.get(
				`${authStore.apiAddress}/api/account/confirm-email?token=${token}`
			)
			.then((response) => {
