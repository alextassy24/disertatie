			...
			<div
				...
				<Form v-if="!successMessage" :validation-schema="validationSchema" @submit="onSubmit" class="mb-3">
					<div class="relative flex flex-col mb-5">
						<label class="form-label">{{ translatedValues.fullName }}</label>
						<Field id="wearerName" type="text" name="wearerName" class="form-field" v-model="wearerName" />
						<ErrorMessage name="wearerName" class="error-message" />
					</div>
					<div class="relative flex flex-col mb-5">
						<label class="form-label">{{ translatedValues.age }}</label>
						<Field id="wearerAge" type="text" name="wearerAge" class="form-field" v-model="wearerAge" />
						<ErrorMessage name="wearerAge" class="error-message" />
					</div>
...
function onSubmit() {
	...
	axios
		.post("http://127.0.0.1:5088/api/wearers/", formData, config)
		.then((response) => {
