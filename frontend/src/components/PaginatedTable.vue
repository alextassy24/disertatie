<template>
    <div class="flex items-center gap-3 mx-2 mt-1 mb-4 text-lg" :class="link ? 'justify-between' : 'justify-end'">
        <div>
            <label>{{ translatedValues.show }}</label>
            <select class="px-1 mx-2 bg-gray-200 rounded-lg focus:ring-2 focus:ring-green-500" v-model="itemsPerPage">
        ...
        <div>
            <router-link v-if="link" :to="link" class="flex items-center gap-2 btn">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <transition-group name="fade" tag="tbody" class="text-center ">
                <tr v-for="(item, index) in paginatedItems" :key="index">
                    ...
                    <td class="flex flex-col items-center justify-center gap-3 lg:flex-row">
                        <router-link :to="`/${itemPage}/${item.id}`" class="btn-primary" tag="button">
                            <i class="fa-solid fa-eye"></i>
                        </router-link>
                        <button class="btn-danger" @click="action(item.id)"><i class="fa-solid fa-trash-can"></i></button>
                    </td>
                </tr>
            </transition-group>
        </table>
    ...
    <div class="flex items-center justify-between m-3 mt-5">
        <div class="flex gap-3">
            <button class="flex items-center gap-1" @click="goToPage(1)"
                :class="currentPage == 1 ? 'btn-disabled' : 'pagination-btn'">
                <i class="fa-solid fa-backward-fast"></i>
                <span class="hidden lg:block">{{ translatedValues.firstBtn }}</span>
            </button>
            <button class="flex items-center gap-1" @click="goToPage(currentPage - 1)" :disabled="currentPage == 1"
                :class="currentPage == 1 ? 'btn-disabled' : 'pagination-btn'">
        ...
        <div>
            <span class="hidden lg:inline-block">{{ translatedValues.tablePage }}</span> {{ currentPage }} {{
                translatedValues.tableOf }} {{ totalPages }}
        </div>
        <div class="flex gap-3">
            <button class="flex items-center gap-1" @click="goToPage(currentPage + 1)"
            ...
            <button class="flex items-center gap-1" @click="goToPage(totalPages)" :disabled="currentPage == totalPages"
                :class="currentPage == totalPages ? 'btn-disabled' : 'pagination-btn'">
