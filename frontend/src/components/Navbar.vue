	...
	<nav
		...
		<div class="container flex items-center justify-between mx-auto">
			<div class="flex items-center gap-1">
				<router-link to="/" active-class="brand-active" class="navbar-brand">
			...
			<div class="hidden md:block">
				<ul class="flex gap-4">
					<li v-for="item in unauthNavigationItems" :key="item.text">
						<router-link class="flex flex-col items-center nav-link" :to="item.to" @click="closeMenu">
							<i :class="item.icon"></i>
					...
					<li @click="toggleDropdown" v-if="authStore.isAuthenticated">
						...
						<div v-if="dropdownMenu" @pointerleave="closeDropdown" class="dropdown">
							<router-link v-for="item in authNavigationItems"
								class="flex flex-row items-center gap-2 dropdown-item" :to="item.to"
								active-class="dropdown-active" @click="closeMenu">
								<i :class="item.icon"></i>
								<span class="mt-1">{{ item.text }}</span>
							</router-link>
						</div>
			...
			<div class="hidden gap-3 md:flex">
				...
				<router-link v-else class="flex items-center gap-2 btn-logout" to="/login" @click="logout">
		...
		<div v-if="collapseMenu" class="block mt-5 md:hidden">
			<ul class="flex flex-col gap-5">
				<li v-for="item in unauthNavigationItems" :key="item.text">
					<router-link class="flex flex-row items-center gap-2 nav-link" :to="item.to" @click="closeMenu">
						<i :class="item.icon"></i>
						<span class="mt-1">{{ item.text }}</span>
					</router-link>
				</li>
				...
				<li>
					<router-link v-if="!authStore.isAuthenticated" class="btn" to="/login"
						@click="closeMenu">Login</router-link>
					<router-link v-else @click="logout" class="btn-logout" to="/login">Logout</router-link>
				</li>
			</ul>
		</div>
	</nav>
...
const authNavigationItems = computed(() => {
	return navigationItems.value.filter(item => item.requireAuth && authStore.isAuthenticated);
});
